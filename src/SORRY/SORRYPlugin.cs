...
public class SORRYPlugin : BaseSpaceWarpPlugin
{
    ...
    float lastRTPCvalue = 1;
    bool hasFailed = false;
    bool initialized;
    void Update()
    {
        if (initialized && !hasFailed && lastRTPCvalue != Time.timeScale)
        {
            var result = AkSoundEngine.SetRTPCValue("timewarp", Time.timeScale);
            switch (result)
            {
                case AKRESULT.AK_Success:
                case AKRESULT.AK_PartialSuccess:
                    lastRTPCvalue = Time.timeScale;
                    break;
                case AKRESULT.AK_Fail:
                    hasFailed = true;
                    break;
            }
        }
